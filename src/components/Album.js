...
class Album extends Component {
  ...
  formatTime(timeInSeconds) {
    const timeString = isNaN(timeInSeconds)
      ? "--:--"
      : parseInt(timeInSeconds / 60, 10) + ":" + Math.round(timeInSeconds % 60);
  ...
  handleSongEnd(ev) {
    this.props.recordSongEnd(this.state.currentSong);
  }
  ...
  componentDidMount() {
    ...
    this.audioElement.addEventListener("volumechange", this.handleVolumeUpdate);
            ...
            >
              <div className="song-number">
                <button className="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--accent">
                  ...
                  <span className="song-number-showing">{index + 1}</span>
                  <i className="material-icons song-pause">pause</i>
                </button>
              </div>
